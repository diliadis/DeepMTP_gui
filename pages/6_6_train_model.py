...
from DeepMTP.utils.utils import (
    generate_config,
    get_default_dropout_rate,
    get_default_batch_norm,
)
...
if (
    ...
    or "selected_gpu" not in st.session_state
    or st.session_state.selected_gpu is None
):
    general_info_placeholder.info(
        "You first have to load a dataset and configure the neural network architecture"
    )

else:
    no_clicking_warning_placeholder.info(
        "Everything seems to be correctly defined. You can click the button below to start training"
    )
    ...
    st.session_state.start_experiment_button_pressed = start_train_button.button(
        "Start training!!!"
    )

    if st.session_state.start_experiment_button_pressed:
        st.warning(
            "The training process has just started. DO NOT click anything in the application, otherwise all progress will be lost!!!"
        )
        ...
        if isinstance(st.session_state.learning_rate, tuple):
            if st.session_state.learning_rate[0] != st.session_state.learning_rate[1]:
                learning_rate = CSH.UniformFloatHyperparameter(
                    "learning_rate",
                    lower=st.session_state.learning_rate[0],
                    upper=st.session_state.learning_rate[1],
                    log=True,
                )
            else:
                learning_rate = CSH.Constant(
                    "learning_rate", value=st.session_state.learning_rate[0]
                )
        else:
            learning_rate = CSH.Constant(
                "learning_rate", value=st.session_state.learning_rate
            )
        ...
        if isinstance(st.session_state.dropout_rate, tuple):
            if st.session_state.dropout_rate[0] != st.session_state.dropout_rate[1]:
                dropout_rate = CSH.UniformFloatHyperparameter(
                    "dropout_rate",
                    lower=st.session_state.dropout_rate[0],
                    upper=st.session_state.dropout_rate[1],
                    log=False,
                )
            else:
                dropout_rate = CSH.Constant(
                    "dropout_rate", value=st.session_state.dropout_rate[0]
                )
        else:
            dropout_rate = CSH.Constant(
                "dropout_rate", value=st.session_state.dropout_rate
            )

        # batch normalization
        if isinstance(st.session_state.batch_norm, tuple):
            if st.session_state.batch_norm[0] != st.session_state.batch_norm[1]:
                batch_norm = CSH.CategoricalHyperparameter(
                    "batch_norm", st.session_state.batch_norm
                )
            else:
                batch_norm = CSH.Constant(
                    "batch_norm", value=st.session_state.batch_norm[0]
                )
        else:
        ...
        if isinstance(st.session_state.instance_branch_nodes_per_layer, tuple):
            if (
                st.session_state.instance_branch_nodes_per_layer[0]
                != st.session_state.instance_branch_nodes_per_layer[1]
            ):
                instance_branch_nodes_per_layer = CSH.UniformIntegerHyperparameter(
                    "instance_branch_nodes_per_layer",
                    lower=st.session_state.instance_branch_nodes_per_layer[0],
                    upper=st.session_state.instance_branch_nodes_per_layer[1],
                    log=False,
                )
            else:
                instance_branch_nodes_per_layer = CSH.Constant(
                    "instance_branch_nodes_per_layer",
                    value=st.session_state.instance_branch_nodes_per_layer[0],
                )
        else:
            instance_branch_nodes_per_layer = CSH.Constant(
                "instance_branch_nodes_per_layer",
                value=st.session_state.instance_branch_nodes_per_layer,
            )

        # number of layers in the instance branch
        if isinstance(st.session_state.instance_branch_layers, tuple):
            if (
                st.session_state.instance_branch_layers[0]
                != st.session_state.instance_branch_layers[1]
            ):
                instance_branch_layers = CSH.UniformIntegerHyperparameter(
                    "instance_branch_layers",
                    lower=st.session_state.instance_branch_layers[0],
                    upper=st.session_state.instance_branch_layers[1],
                    log=False,
                )
            else:
                instance_branch_layers = CSH.Constant(
                    "instance_branch_layers",
                    value=st.session_state.instance_branch_layers[0],
                )
        else:
            instance_branch_layers = CSH.Constant(
                "instance_branch_layers", value=st.session_state.instance_branch_layers
            )

        # number of nodes in every layer of the target branch
        if isinstance(st.session_state.target_branch_nodes_per_layer, tuple):
            if (
                st.session_state.target_branch_nodes_per_layer[0]
                != st.session_state.target_branch_nodes_per_layer[1]
            ):
                target_branch_nodes_per_layer = CSH.UniformIntegerHyperparameter(
                    "target_branch_nodes_per_layer",
                    lower=st.session_state.target_branch_nodes_per_layer[0],
                    upper=st.session_state.target_branch_nodes_per_layer[1],
                    log=False,
                )
            else:
                target_branch_nodes_per_layer = CSH.Constant(
                    "target_branch_nodes_per_layer",
                    value=st.session_state.target_branch_nodes_per_layer[0],
                )
        else:
            target_branch_nodes_per_layer = CSH.Constant(
                "target_branch_nodes_per_layer",
                value=st.session_state.target_branch_nodes_per_layer,
            )

        # number of layers in the target branch
        if isinstance(st.session_state.target_branch_layers, tuple):
            if (
                st.session_state.target_branch_layers[0]
                != st.session_state.target_branch_layers[1]
            ):
                target_branch_layers = CSH.UniformIntegerHyperparameter(
                    "target_branch_layers",
                    lower=st.session_state.target_branch_layers[0],
                    upper=st.session_state.target_branch_layers[1],
                    log=False,
                )
            else:
                target_branch_layers = CSH.Constant(
                    "target_branch_layers",
                    value=st.session_state.target_branch_layers[0],
                )
        else:
            target_branch_layers = CSH.Constant(
                "target_branch_layers", value=st.session_state.target_branch_layers
            )
        ...
        if isinstance(st.session_state.embedding_size, tuple):
            if st.session_state.embedding_size[0] != st.session_state.embedding_size[1]:
                embedding_size = CSH.UniformIntegerHyperparameter(
                    "embedding_size",
                    lower=st.session_state.embedding_size[0],
                    upper=st.session_state.embedding_size[1],
                    log=False,
                )
            else:
                embedding_size = CSH.Constant(
                    "embedding_size", value=st.session_state.embedding_size[0]
                )
        else:
            embedding_size = CSH.Constant(
                "embedding_size", value=st.session_state.embedding_size
            )
        ...
        if False not in [
            isinstance(hyperparam, CSH.Constant)
            for hyperparam in cs.get_hyperparameters()
        ]:
            ...
            config = generate_config(
                instance_branch_input_dim=st.session_state.data_info[
                    "instance_branch_input_dim"
                ],
                target_branch_input_dim=st.session_state.data_info[
                    "target_branch_input_dim"
                ],
                validation_setting=st.session_state.data_info[
                    "detected_validation_setting"
                ],
                general_architecture_version="dot_product",  # this can be updated after I create something like a "general architecture version" option in configure_neural_network.py
                problem_mode=st.session_state.data_info["detected_problem_mode"],
                learning_rate=sampled_config["learning_rate"],
                decay=0,
                batch_norm=batch_norm,
                dropout_rate=get_default_dropout_rate()
                if "dropout_rate" not in sampled_config
                else sampled_config["dropout_rate"],
                momentum=0.9,
                weighted_loss=False,
                compute_mode=st.session_state["selected_gpu"]
                if st.session_state["selected_gpu"] == "cpu"
                else "cuda:" + st.session_state["selected_gpu"],
                train_batchsize=512,
                val_batchsize=512,
                ...
                else st.session_state["use_wandb"]["project"],
                wandb_project_entity=None
                if st.session_state["use_wandb"] is None
                else st.session_state["use_wandb"]["entity"],
                metric_to_optimize_early_stopping="loss",
                metric_to_optimize_best_epoch_selection=st.session_state.config[
                    "metrics"
                ][0],
                ...
                instance_branch_params={
                    ...
                    "instance_branch_nodes_per_layer": sampled_config[
                        "instance_branch_nodes_per_layer"
                    ],
                ...
                target_branch_params={
                    ...
                    "target_branch_nodes_per_layer": sampled_config[
                        "target_branch_nodes_per_layer"
                    ],
            ...
            with st.spinner("training..."):
                validation_results = model.train(
                    st.session_state.train, st.session_state.val, st.session_state.test
                )
            ...
            if (
                not st.session_state.hyperband_selected
                and not st.session_state.random_search_selected
            ):
                ...
                m_to_optimize = (
                    streamlit_to_deepMTP_metrics_map[
                        st.session_state.hpo_metric_to_optimize
                    ]
                    if st.session_state.hpo_metric_to_optimize != "loss"
                    else st.session_state.hpo_metric_to_optimize
                )
                m_to_optimize = (
                    m_to_optimize
                    + "_"
                    + st.session_state.hpo_metric_average_to_optimize
                    if m_to_optimize != "loss"
                    else m_to_optimize
                )
                ...
                if st.session_state.hyperband_selected:
                    ...
                    opt = HyperBand(
                        ...
                        max_budget=st.session_state.max_budget,
                        direction="max"
                        if st.session_state.hpo_metric_to_optimize
                        in [
                            "accuracy",
                            "recall",
                            "precision",
                ...
                best_model = DeepMTP(
                    best_overall_config.info["config"],
                    best_overall_config.info["model_dir"],
                )
                # use the model to generate predictions on the test set and calculate the performance metrics
                best_model_results = best_model.predict(
                    st.session_state.test, verbose=True
                )
                # print the performance metrics
                st.subheader(
                    "Performance metrics on the test set using the best model: "
                )
                st.json(
                    best_model_results[0]
                )  # selects only the metrics and not the actuall predictions
